            ...
            if (!mapname.empty())
              dynamic_cast<ExitPoint *>(actor)->setMap(mapname);
					...
					if(prop != NULL) {
						...
						std::string attrname = ((TiXmlElement*)prop)->Attribute("name");
						if(attrname == "name")
							objectName = ((TiXmlElement*)prop)->Attribute("value");
						if (!objectName.empty()) {
							cout << "  " << objectName << " " << x << " " << y << "\n";
							QScriptValue args = scriptEngine->newArray(2);
							args.setProperty(0, QScriptValue(x));
							args.setProperty(1, QScriptValue(y));
							scriptEngine->globalObject().property(objectName.data()).call(QScriptValue(), args);

							if(scriptEngine->hasUncaughtException()) {
								//cprint(filename + ": " + QString::number(scriptEngine->uncaughtExceptionLineNumber()) + ": " +
								//				scriptEngine->uncaughtException().toString());
								qDebug() << QString::number(scriptEngine->uncaughtExceptionLineNumber()) << ": " << scriptEngine->uncaughtException().toString();
								scriptEngine->clearExceptions();
							}
						}
					}
				} else {
					// Check if there's a script function

					QScriptValue func = scriptEngine->globalObject().property(type.data());
					if(func.isFunction()) {
						QScriptValue args = scriptEngine->newArray(2);
						args.setProperty(0, QScriptValue(x));
						args.setProperty(1, QScriptValue(y));
						scriptEngine->globalObject().property(type.data()).call(QScriptValue(), args);
...
static int map_begin_collide(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  Actor *actor2 = (Actor *) b->data;
	...
	actor1->collideCallback(*actor2);
	actor2->collideCallback(*actor1);
  return 1;
}
...
static int map_colliding(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  //cout << "Collision: " << actor1->actorName << " " << actor2->actorName << "\n";
  actor1->collidingCallback(*actor2);
  actor2->collidingCallback(*actor1);
  return 1;
}
...
static int map_bumper_begin_collide(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  Actor *actor = bumper->actor;
	...
	//if(dynamic_cast<ScriptedActor *>(actor))
	//	cout << "Bumper collision: " << actor->actorName << "\n";
  actor->bumperCollideCallback(bumper->facing_direction);
  return 1;
}
...
static int map_bumper_colliding(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  //cout << "Collision: " << actor1->actorName << " " << actor2->actorName << "\n";
  actor->bumperCollidingCallback(bumper->facing_direction);
  return 1;
}
...
static void map_bumper_end_collide(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  //cout << "Collision: " << actor1->actorName << " " << actor2->actorName << "\n";
  actor->bumperEndCollideCallback(bumper->facing_direction);
}
...
static int map_ground_collide(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  cpVect normal = cpArbiterGetNormal(arb, 0);
	...
	cpVect groundVelocity = b->body->v;
  ...
  if(normal.y < 0) {
    ...
    if(slope <= 2)
			actor1->collideGround(groundVelocity);
    else
      actor1->collideWall();
  } else {
    actor1->collideWall();
...
static int map_bumper_begin_ground_collide(cpArbiter *arb, cpSpace *space, void *data)
{
  ...
  Bumper *bumper = (Bumper *) a->data;
	...
	//if(dynamic_cast<ScriptedActor *>(bumper->actor))
