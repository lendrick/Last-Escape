	...
	: ScriptedActor(x, y, "xeon.xml")
{
  ...
  dying = false;
  canSleep = false;
	shapeLayers = PhysicsLayer::Map|PhysicsLayer::Player|PhysicsLayer::Enemy|PhysicsLayer::EnemyBullet;
	collisionType =  PhysicsType::Player;
...
void Player::jump(double dt)
{
  ...
  //cout << "jumping\n";
	...
	if (isGrounded() && time - last_jump_time > jump_wait && input.jump()) {
    //cout << "jump!\n";
    if (energy < energy_cost_jump)
      return;

		//cpBodyApplyImpulse(body, cpv(0, 6000), cpv(0, 0));
		body->v.y = groundVelocity.y + jumpVelocity;
    last_jump_time = time;
    jumpSound->play();
  } else {
    //if(isGrounded())
...
void Player::shoot()
{
  ...
  if (time - last_shoot_time > weapons[currentWeapon].reload_time) {
    ...
    new PlayerBullet(bulletX, bulletY, facing_direction, weapons[currentWeapon].angle_variation);
  ...
  if(!dying) {
    ...
    if(time - last_shoot_time < shoot_duration) {
      ...
      this->setCurrentAnimation("idle");
		...
		//cout << "Updated sprite facing " << facing_direction << "\n";
    updateSpriteFacing();
...
void Player::collidingCallback(Actor & otherActor)
{
  if(!dying) {
...
void Player::destroyCallback()
{
  if (debugMode)
    cout << "player destroyed\n";
}

void Player::animationCompleteCallback(std::string anim)
{
  //cout << "EnemyWalker::animationCompleteCallback(\"" << anim << "\")\n";
  if(anim == "die") {
    if(lives > 0) {
      ...
      new CollectibleEnergyBall(body->p.x - 16, body->p.y - 48);
...
void Player::damageCallback()
{
  soundCache["grunt.ogg"]->play();
}
...
void Player::setCurrentExperience(int e)
{
	currentExperience = e;
}
